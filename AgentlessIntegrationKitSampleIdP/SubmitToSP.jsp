...
try
{
	...
	userAttributes.put( "subject", request.getParameter("subject") );
	userAttributes.put( "name", request.getParameter("name"));
	userAttributes.put( "status", request.getParameter("status"));
	
	// 2 - Send the user attributes to the referenceID adapter and get the returned reference value
	
	String idpReferenceValue = dropOffAttributesToRefererenceIdAdapter( userAttributes );
	
	// 3 - Redirect to PF to start SSO to partner, passing the attribute reference value
	
	response.sendRedirect(PF_BASE_URL + ":" + PF_PRIMARY_SSL_PORT + PF_START_SSO_ENDPOINT + "?" + "PartnerSpId=" + PARTNER_ENTITY_ID + "&REF=" + idpReferenceValue);
}
catch (Exception e)
{
